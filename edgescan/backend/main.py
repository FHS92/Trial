...
def get_portfolio_metrics(authorization: str = Header(default=""), db: Session = Depends(get_db)):
    ...
    date_closes: dict[date, dict[str, float]] = {}
    for r in ph_rows:
        if r.date not in date_closes:
            date_closes[r.date] = {}
        if r.close is not None:
            date_closes[r.date][r.ticker] = r.close
...
# ---------------------------------------------------------------------------
# Scan movers — score changes between last two scans per ticker
# ---------------------------------------------------------------------------

@app.get("/api/scan/movers")
def get_scan_movers(top_n: int = 10, db: Session = Depends(get_db)):
    ...
    for row in rows:
        ...
        if sample_current is None:
            sample_current = _ts(row.current_ts)
            sample_previous = _ts(row.prev_ts)
...
# ---------------------------------------------------------------------------
# Portfolio price refresh + professional metrics
# ---------------------------------------------------------------------------

@app.post("/api/portfolio/refresh-prices")
def refresh_portfolio_prices(authorization: str = Header(default=""), db: Session = Depends(get_db)):
    """Download 1-year price history for all holdings + ^GSPC and upsert into price_history."""
    import math as _math
    import yfinance as yf
    import pandas as pd
    ...
    if not holdings:
        return {"status": "ok", "rows_updated": 0, "message": "No holdings to refresh."}

    tickers_to_refresh = list({h.ticker for h in holdings}) + ["^GSPC"]
    count = 0
    ...
    for t in tickers_to_refresh:
        try:
            ...
            print(f"[refresh-prices] Error for {t}: {exc}")
