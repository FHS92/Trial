  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <span className="font-bold text-sm" style={{ color: 'var(--color-text)' }}>{m.ticker}</span>
        <p className="text-xs truncate" style={{ color: 'var(--color-text-2)' }}>{m.name}</p>
      </div>
      {m.current_price && (
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-bg)' }}>
      ...
      >
        ...
        <span className="font-semibold text-sm" style={{ color: 'var(--color-text)' }}>Weekly Snapshot</span>
      </header>

      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        <div className="mb-6">
          <h1 className="text-xl font-bold mb-1" style={{ color: 'var(--color-text)' }}>Weekly Snapshot</h1>
        ...
        {!data ? (
          <p className="text-sm text-center py-16" style={{ color: 'var(--color-text-2)' }}>Could not load snapshot.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">

            {/* Top scores */}
            <div className="sm:col-span-2 rounded-xl overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
              <div className="px-4 py-3" style={{ background: 'var(--color-card-alt)', borderBottom: '1px solid var(--color-border)' }}>
                <h2 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Top 10 This Week</h2>
              </div>
              <div className="grid sm:grid-cols-2">
                {data.top_stocks.map((m, i) => (
                  <div key={m.ticker} style={{ borderRight: i % 2 === 0 ? '1px solid var(--color-border)' : 'none' }}>
            ...
            <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(34,197,94,0.15)' }}>
              <div className="px-4 py-3" style={{ background: 'rgba(34,197,94,0.05)', borderBottom: '1px solid rgba(34,197,94,0.1)' }}>
                ...
                {!hasDeltas && <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-2)' }}>Available after 7+ days of scans</p>}
              </div>
              <div>
                {hasDeltas
                  ? data.gainers.map(m => <MoverRow key={m.ticker} m={m} />)
                  : <p className="text-xs px-4 py-8 text-center" style={{ color: 'var(--color-text-2)' }}>No data yet — check back in a week</p>
                }
              </div>
            </div>
            ...
            {data.sectors.length > 0 && (
              <div className="sm:col-span-2 rounded-xl p-5" style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
                <h2 className="text-sm font-semibold mb-4" style={{ color: 'var(--color-text)' }}>Sector Avg Score</h2>
                {data.sectors.map(s => (
