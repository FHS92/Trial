    ...
    <div className="min-h-screen" style={{ background: 'var(--color-bg)' }}>
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        <div className="mb-6">
          ...
          <p className="text-sm" style={{ color: 'var(--color-text-2)' }}>Upcoming earnings for S&P 500 stocks in the scanner — next 45 days</p>
        </div>
            ...
            {dates.map(dateStr => (
              <div key={dateStr}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: 'var(--color-border)', color: urgencyColor(dateStr) }}>
                    {daysUntil(dateStr)}
                  </span>
                </div>
                <div className="rounded-xl overflow-hidden" style={{ border: '1px solid var(--color-border)' }}>
                  {grouped[dateStr].map((item, i) => (
                    <Link
                      ...
                      style={{ borderBottom: i < grouped[dateStr].length - 1 ? '1px solid var(--color-border)' : 'none', background: i % 2 === 0 ? 'var(--color-card)' : '#0b1019' }}
                    >
                      ...
                      <div className="text-right flex-shrink-0">
                        {item.current_price && <p className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>${item.current_price.toFixed(2)}</p>}
                      ...
                      <div className="flex-shrink-0 w-10 text-center">
                        ...
                        <p className="text-xs" style={{ color: 'var(--color-text-2)' }}>score</p>
                      </div>
                    </Link>
                  ))}
                </div>
