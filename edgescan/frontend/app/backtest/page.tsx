...
interface Holding { ticker: string; score: number; entry: number; exit: number; return_pct: number; pnl: number; carried?: boolean; stopped?: boolean }
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        <div className="mb-5">
          ...
          <p className="text-sm" style={{ color: '#6b7a99' }}>
            Jan 2020 → today · Top 3 picks · $6,000 start · Carry + 10% stop-loss · vs SPY buy-and-hold
          </p>
        </div>
            ...
            <div>
              ...
              <div className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.06)' }}>
                ...
                {data.monthly.map((r, i) => {
                  const picks = r.holdings.map(h => `${h.ticker}(${h.score})${h.carried ? '↻' : ''}${h.stopped ? '✕' : ''}`).join(' · ')
                  return (
