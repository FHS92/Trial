    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-4">
      <div className="flex items-center justify-between gap-4">
            ...
            <div className="mt-1.5 space-y-1">
              <div className="flex items-center justify-between">
                <p className="text-xs text-[var(--text-muted)]">
                  {items.length} of {limit} slots used
                </p>
                <p className="text-xs text-[var(--text-muted)]">Free plan</p>
              </div>
              <div className="h-1.5 w-full max-w-[160px] rounded-full bg-[var(--border)] overflow-hidden">
                <div
                  className="h-full rounded-full transition-all duration-500"
                  style={{
                    width: `${Math.min(100, (items.length / limit) * 100)}%`,
                    backgroundColor: items.length >= limit ? '#ef4444' : items.length >= limit * 0.8 ? '#f59e0b' : 'var(--accent)',
                  }}
                />
              </div>
            </div>
        ...
        {isFree && (
          <Link
            href="/upgrade"
            className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-white shrink-0"
            style={{ backgroundColor: 'var(--accent)' }}
          >
      ...
      {items.length === 0 ? (
        <div className="space-y-4">
          <div className="flex flex-col items-center justify-center py-10 px-4 text-center rounded-xl border border-dashed border-[var(--border)]">
          ...
          {suggestions.length > 0 && (
            <div>
              ...
              <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] overflow-hidden">
                {suggestions.map((s) => (
                  <div key={s.ticker} className="flex items-center gap-3 px-4 py-3 border-b border-[var(--border)] last:border-b-0 hover:bg-[var(--border)]/30 transition-colors">
                    <ScoreRing score={s.score} size={40} />
                    <Link href={`/stock/${s.ticker}`} className="flex-1 min-w-0 group">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-sm text-[var(--text)] group-hover:text-[var(--accent)] transition-colors">{s.ticker}</span>
                        <ScoreBadge score={s.score} />
                      </div>
                      <p className="text-xs text-[var(--text-muted)] truncate mt-0.5">{s.name}</p>
                    </Link>
                    <div className="flex flex-col items-end gap-0.5 shrink-0">
                      <span className="text-sm font-mono font-semibold text-[var(--text)] tabular-nums">{formatPrice(s.current_price)}</span>
                      {s.upside_pct != null && (
                        <span className={cn('text-xs font-mono tabular-nums', (s.upside_pct ?? 0) >= 0 ? 'text-[#22c55e]' : 'text-[#ef4444]')}>
                    ...
                    <button
                      ...
                      className="shrink-0 p-1.5 rounded-md text-[var(--text-muted)] hover:text-[var(--accent)] hover:bg-[var(--accent)]/10 transition-colors"
