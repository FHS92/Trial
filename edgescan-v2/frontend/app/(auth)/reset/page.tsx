    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(
          body?.detail ??
          body?.error?.message ??
          (res.status === 400 ? 'Invalid or expired reset link.' : 'Reset failed. Please try again.')
        )
      }
    } catch {
  ...
  return (
    <div className="bg-[#1a1d27] border border-[#2a2d3a] rounded-xl p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className={`w-full bg-[#0f1117] border rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:outline-none transition-colors ${
              pwMismatch ? 'border-red-500 focus:border-red-500' : 'border-[#2a2d3a] focus:border-indigo-500'
            }`}
          />
          {pwMismatch && (
        ...
        <button
          type="submit"
          disabled={loading || !token || pwMismatch}
